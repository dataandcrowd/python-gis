...
4. **CRS test.** Confirm the function preserves or sets the expected CRS. For most A3 briefs this should be `EPSG:2193` (NZTM) for spatial maths or `EPSG:4326` (WGS 84) for input from raw lat/lon.
...
5. **Fixture test** (option A). A reusable test sample shared across tests.
...
[Take a look at the poster](data/poster-example-hshin.pdf)
