...
You need to put your project under version control and push it to a public GitHub repository. There are two ways to do this: via the **terminal** (Option A) or via **GitHub Desktop** (Option B). Pick whichever you are more comfortable with; the end result is the same.
...
If you prefer a GUI, [GitHub Desktop](https://desktop.github.com/) does all of the above with clicks instead of commands.
...
::: {.callout-tip}
## If the push is rejected (terminal) or the publish fails (GitHub Desktop)
The most common cause is that the remote repo is not empty (there is already a README on GitHub). Either delete the remote and recreate it empty, or (terminal only) pull first with `git pull origin main --allow-unrelated-histories` and resolve any conflicts before pushing again. In GitHub Desktop, start over with a fresh empty repo.
:::
