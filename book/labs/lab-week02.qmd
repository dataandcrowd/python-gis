...
- The file `akl_ped-2024.csv` (https://github.com/dataandcrowd/GISCI343/tree/main/pedestrians)
...
# Part 2.1: Python Foundations Review {.unnumbered}
...
**Your turn:** Create a list of the hour labels (e.g., `"6:00-6:59"`, `"7:00-7:59"`, ..., `"21:00-21:59"`) using a list comprehension. Hint: `[f"{h}:00-{h}:59" for h in range(6, 22)]`.
...
**Your turn:** Write a loop that prints each sensor location alongside the percentage of the total peak count it represents. For example, "107 Quay Street: 33.4%".
...
**Your turn:** Modify the thresholds so that "HIGH" is above 800, "MEDIUM" is 200-800, and "LOW" is below 200. How does the classification change?
...
**Your turn:** Find the two consecutive hours with the highest combined count. For example, if hour 12 has 1,335 and hour 13 has 1,423, their combined count is 2,758. Which consecutive pair is the busiest?

---

# Part 2.2: DataFrames and Pandas {.unnumbered}
...
Real sensor data almost always has gaps: equipment failures, maintenance windows, or transmission errors.

```{python}
# Check missing values per column
missing = ped[sensor_cols].isna().sum()
missing_pct = (missing / len(ped) * 100).round(1)
...
```{python}
# Strategy 1: Fill with 0 (assumes no sensor reading = no people)
ped_zero = ped[sensor_cols].fillna(0)

# Strategy 2: Forward fill (carry last known value)
ped_ffill = ped[sensor_cols].ffill()

# Strategy 3: Interpolate (estimate between known values)
ped_interp = ped[sensor_cols].interpolate()

# Strategy 4: Drop rows with any missing
ped_clean = ped.dropna(subset=sensor_cols.tolist())
print(f"Rows after dropping missing: {len(ped_clean)} (from {len(ped)})")
```

::: {.callout-note}
## Which Strategy?
There is no single correct answer. Filling with zero assumes sensor silence means no pedestrians, which may undercount. Forward fill assumes counts stay constant, which may overcount. Interpolation is often a reasonable middle ground for continuous time series. Always document and justify your choice.
:::

**Your turn:** For a single sensor of your choice, compare the mean count using each of the four strategies above. How much do the results differ?
...
# Part 2.3: Data Visualisation {.unnumbered}
