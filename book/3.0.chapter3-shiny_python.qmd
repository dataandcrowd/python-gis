# Interactive Dashboards with Shiny for Python {#sec-shiny-python}

## Overview

Web-based dashboards have revolutionised how we communicate geospatial analysis. Instead of static maps and reports, dashboards allow users to explore data dynamically—filtering, zooming, and interacting with visualisations in real time. This chapter introduces **Shiny for Python**, a powerful framework for building interactive web applications without requiring extensive web development experience.

You'll learn to create professional dashboards that combine interactive maps, responsive controls, and analytical outputs—perfect for communicating urban analytics insights to policymakers, stakeholders, and the public.

## Story Problem: An Auckland Dashboard

**From Static to Interactive**

In Chapter 2, you created choropleth maps showing urban indicators across Auckland. These maps told a clear story, but were limited:
...
1. **WebGIS Fundamentals** (@sec-webgis-wasm-shiny): Understanding web mapping, file formats, and WebAssembly (WASM) for browser-based applications

2. **Shiny Architecture** (@sec-shiny-functions-reactivity): The UI/server model, reactive programming, and building responsive interfaces

3. **Routing and Congestion** (@sec-routing-congestion): Analysing transport networks, travel times, and congestion indicators

4. **Origin-Destination Analysis** (@sec-origin-destination): Understanding and visualising movement patterns between locations

5. **Network Analysis** (@sec-networks-gis): Working with street networks using OSMnx and city2graph for accessibility analysis

6. **Dashboard Development** (@sec-shiny-assignment): Building and deploying complete multi-page applications
...
- Combine with spatial tools: folium, ipyleaflet, OSMnx
...
- **Air Quality Monitoring**: Temporal and spatial patterns with filtering by pollutant type
- **E-Scooter Demand Analysis**: Trip patterns by time of day and location
- **Before/After Policy Evaluation**: Comparing urban indicators pre- and post-intervention

## Chapter Structure

### Part 1: Foundations (Week 6)
...
**Focus**: Building your Assignment 2 dashboard with instructor support
